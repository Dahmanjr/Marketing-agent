...
def analyze_product_and_generate_script(
    image_bytes: bytes, tone: str, duration: int, extra_info: str
) -> dict:
...
def create_ad_frame(
    product_img, headline, subtext, tagline,
    palette, frame_index, total_frames, product_name,
):
    W, H = 1080, 720
    ...
    if frame_index == total_frames - 1:
        ...
        draw.rectangle([text_x-5, tag_y-8, text_x+tw+15, tag_y+30], fill=(*accent_rgb, 30))
...
def create_video_from_frames(frames, output_path, fps=24, frame_duration=3):
        ...
        with st.status("🤖 Gemini is analyzing your product…", expanded=True) as status:
            ...
            try:
                script_data = analyze_product_and_generate_script(
                    jpeg_bytes, tone, duration, extra_info
                )
